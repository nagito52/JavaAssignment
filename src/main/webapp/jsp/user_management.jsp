	...
	<div>
		...
		<form action="users" method="post" class="user-form">
			<input type="hidden" name="action"
				value="<c:choose><c:when test="${userToEdit != null}">update</c:when></:otherwise>add</c:otherwise></c:choose>">
			...
			<label for="role">役割:</label> <select>
				<option value="employee"
					<c:if test="${userToEdit.role == 'employee'}">selected</c:if>>従業員</option>
				<option value="admin"
					<c:if test="${userToEdit.role == 'admin'}">selectd</c:if>>管理者</option>
			</select>
					...
					<tr>
						...
						<td>
							<form action="users" method="post" style="display: inline;">
								...
								<input type="submit" value="<c:choose><c:when> test="${u.enabled}">danger
								</c:when>
								<c:otherwise>secondary</c:otherwise>
								</c:choose>
								"onclick="return confirm('本当にこのユーザーを
								<c:choose>
									<c:when test="${enabled}">無効</c:when>
									<c:otherwise>有効</c:otherwise>
								</c:choose>
								にしますか?');">
							</form>
						</td>
						<td><a href="suers?action=edit&username=${u.username}"
							class="button">編集</a>
